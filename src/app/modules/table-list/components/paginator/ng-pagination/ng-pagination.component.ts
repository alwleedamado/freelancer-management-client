...
// fork of https://github.com/ng-bootstrap/ng-bootstrap/blob/master/pagination/pagination.ts
import {
  Component,
  ContentChild,
  Directive,
...
export class NgPagination implements OnChanges {
  ...
  selectPage(pageNumber: number): void { this._updatePages(pageNumber, true); }

  ngOnChanges(changes: SimpleChanges): void { this._updatePages(this.page, false); }
  ...
  private _updatePages(newPage: number, emitChanges: boolean) {
    ...
    this.page = getValueInRange(newPage, this.pageCount, 1);

    if (emitChanges && (this.page !== prevPageNo && isNumber(this.collectionSize))) {
